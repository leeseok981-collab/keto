...
if (code.match(speedModalRegex)) {
    code = code.replace(speedModalRegex, genericModal);
} else {
    // If not found, it means previous script worked. Let's just append the genericModal if not present.
    if (!code.includes('fixed inset-0 bg-black/90 z-[100]')) {
        const insertAfter = '{/* Naro Shop Modal (Google Play Style) */}';
        code = code.replace(insertAfter, genericModal + '\n' + insertAfter);
    }
}
...
if (hackInputMatch) code = code.replace(hackInputRegex, "");
if (hackWarningMatch) code = code.replace(hackWarningRegex, "");
if (executorMatch) code = code.replace(executorRegex, "");
...
if (naroMatch && (hackInputMatch || hackWarningMatch || executorMatch)) {
    ...
    console.log("Could not find Naro shop end or UIs to inject");
}
