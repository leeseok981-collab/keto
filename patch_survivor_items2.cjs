...
const inventoryTabRegex = /\{menuTab === 'inventory' && \(\s*<div>\s*<h2 className="text-3xl font-black text-white mb-6 flex items-center gap-2"><Package className="w-8 h-8 text-blue-400"\/> 내 특공대<\/h2>[\s\S]*?<\/div>\s*\)\}/;

const newInventoryUI = `{menuTab === 'inventory' && (
...
code = code.replace(inventoryTabRegex, newInventoryUI);
