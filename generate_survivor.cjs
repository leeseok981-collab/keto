...
const WORLDS = [
    { id: 1, name: '지구', emoji: '🌍', clearTime: 60 },
    { id: 2, name: '화성', emoji: '🪐', clearTime: 60 }
];
...
export default function SurvivorGame({ user, userData, onBack, deviceMode }: any) {
    ...
    const [stats, setStats] = useState({ hp: 100, maxHp: 100, level: 1, xp: 0, maxXp: 10, kills: 0, time: 0 });
    ...
    const handleGacha = () => {
        ...
        saveMeta({ survivorCoins: coins - 500, survivorSkins: newSkins });
    };
    ...
    const spawnEnemy = (isBoss = false) => {
        ...
        const minutes = Math.floor(eng.gameTime / 60);
        
        if (isBoss) {
            const hp = 500 + minutes * 500;
            eng.enemies.push({ id: Math.random(), x: ex, y: ey, hp, maxHp: hp, speed: 70, damage: 20, color: '#9333ea', radius: 30, isBoss: true });
        } else {
            const hp = 20 + minutes * 15;
            const speed = 40 + Math.random() * 20 + minutes * 5;
            const damage = 5 + minutes * 5;
    ...
    const updateGame = (dt: number) => {
        ...
        // Boss & Spawning (초반 조금, 20초마다 보스)
        const sec = Math.floor(eng.gameTime);
        if (sec > 0 && sec % 20 === 0 && sec !== eng.lastBossTime) {
            eng.lastBossTime = sec;
            spawnEnemy(true);
            eng.enemies = eng.enemies.filter(e => e.isBoss); // 보스 등장 시 일반 몹 제거
        } else if (!eng.enemies.some(e => e.isBoss)) {
            const spawnRate = Math.max(0.2, 3.0 - (eng.gameTime / 60)); // 3초 -> 0.2초
            if (Math.random() < dt / spawnRate) spawnEnemy();
            if (eng.gameTime > 30 && Math.random() < dt * 0.1) {
        ...
        } else if (eng.gameTime >= eng.targetTime) {
            eng.status = 'victory';
        ...
        if (Math.random() < 0.1) {
            setStats({ hp: Math.ceil(eng.player.hp), maxHp: Math.ceil(eng.player.maxHp), level: eng.player.level, xp: eng.player.xp, maxXp: eng.player.maxXp, kills: eng.player.kills, time: Math.floor(eng.gameTime) });
        }
    };
    ...
    const startGame = () => {
        ...
        engine.current = {
            ...
            targetTime: WORLDS[currentWorldIdx].clearTime, bossSpawned: false, lastBossTime: 0
        };
        setPlayerSkills({ [skinData.weapon]: 1 });
        setStats({ hp: 100, maxHp: 100, level: 1, xp: 0, maxXp: 10, kills: 0, time: 0 });
        setGameState('playing');
        setMode('playing');
        requestAnimationFrame(gameLoop);
    };
                        ...
                        <div className="h-full flex flex-col items-center justify-center">
                            <div className="flex items-center gap-8 mb-12">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-slate-400 font-bold mb-4">목표: {WORLDS[currentWorldIdx].clearTime}초 생존</p>
            ...
            <div className="absolute top-0 inset-x-0 p-4 flex justify-between items-start z-10 pointer-events-none">
                ...
                {(gameState === 'playing' || gameState === 'levelup') && (
                    <div className="flex-1 flex flex-col items-center px-4">
                        <div className="text-3xl font-black text-white drop-shadow-lg mb-2 flex items-center gap-2">
                            {formatTime(stats.time)} <span className="text-sm text-slate-400">/ {formatTime(WORLDS[currentWorldIdx].clearTime)}</span>
                        </div>
                        <div className="w-full max-w-md bg-slate-800/80 p-2 rounded-xl backdrop-blur-sm shadow-lg pointer-events-auto">
                            ...
                            <div className="mt-2 flex justify-between text-xs font-bold text-slate-300">
                                ...
                                <span className="flex items-center gap-1"><Skull className="w-3 h-3 text-slate-400"/> {stats.kills}</span>
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="absolute inset-0 bg-black/90 flex items-center justify-center p-4 z-30">
                    <div className="bg-slate-900 p-8 rounded-3xl border-2 border-slate-700 text-center max-w-sm w-full">
                        ...
                        <div className="bg-slate-800 p-4 rounded-xl mb-6 text-left space-y-2">
                            <div className="flex justify-between"><span className="text-slate-400">생존 시간:</span> <span className="text-white font-bold">{formatTime(stats.time)}</span></div>
...
fs.writeFileSync('src/SurvivorGame.tsx', newCode);
