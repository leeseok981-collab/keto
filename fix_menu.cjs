...
const startIndex = code.indexOf("if (mode === 'menu') {");
const endIndex = code.indexOf("return (", startIndex + 20) - 4; // Right before the next return

// Just replace everything from startIndex to endIndex
const newMenu = `if (mode === 'menu') {
        return (
...
code = code.slice(0, startIndex) + newMenu + code.slice(endIndex);
