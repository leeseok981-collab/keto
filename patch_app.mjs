...
// Add the Keycap Clicker game modal
const blueTowerModal = /\{selectedGame === 'blue_tower' && \([\s\S]*?\}\)/;
const keycapModal = `{selectedGame === 'keycap_clicker' && (
...
code = code.replace(blueTowerModal, (match) => match + '\n' + keycapModal);

// Add the Keycap Clicker game card
const blueTowerCard = /<div onClick=\{\(\) => setSelectedGame\('blue_tower'\)\}[\s\S]*?<\/div>\n                                <\/div>/;
const keycapCard = `<div onClick={() => setSelectedGame('blue_tower')} className="bg-slate-900 rounded-2xl p-4 border border-slate-700 hover:border-blue-500 hover:bg-slate-800 cursor-pointer transition-all text-center group">
...
code = code.replace(blueTowerCard, keycapCard);

fs.writeFileSync('src/App.tsx', code);
console.log("App patched for Keycap game");
