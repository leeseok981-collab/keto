...
// 1. Remove GameDetailModal import
code = code.replace("import { GameDetailModal } from './components/GameDetailModal';\n", "");

// 2. Revert Game Cards
const speedKeyboardCard = `
                                <div onMouseEnter={sound.hover} onClick={() => { sound.click(); setSelectedGame('speed_keyboard'); }} className="bg-slate-900 rounded-2xl p-4 border border-slate-700 hover:border-indigo-500 hover:bg-slate-800 cursor-pointer transition-colors text-center group">
...
`;
code = code.replace(/<div onMouseEnter=\{sound\.hover\} onClick=\{\(\) => \{ sound\.click\(\); setSelectedGame\('speed_keyboard'\); \}\}.*?<img src=\{GAME_DETAILS\['speed_keyboard'\]\.icon\}.*?<\/div>.*?<\/div>/s, speedKeyboardCard.trim());
...
`;
code = code.replace(/<div onMouseEnter=\{sound\.hover\} onClick=\{\(\) => \{ sound\.click\(\); setSelectedGame\('fishing'\); \}\}.*?<img src=\{GAME_DETAILS\['fishing'\]\.icon\}.*?<\/div>.*?<\/div>/s, fishingCard.trim());
...
`;
code = code.replace(/<div onMouseEnter=\{sound\.hover\} onClick=\{\(\) => \{ sound\.click\(\); setSelectedGame\('garden'\); \}\}.*?<img src=\{GAME_DETAILS\['garden'\]\.icon\}.*?<\/div>.*?<\/div>/s, gardenCard.trim());

const blueTowerCard = `
                                <div onMouseEnter={sound.hover} onClick={() => { sound.click(); setSelectedGame('blue_tower'); }} className="bg-slate-900 rounded-2xl p-4 border border-slate-700 hover:border-blue-500 hover:bg-slate-800 cursor-pointer transition-colors text-center group">
...
`;
code = code.replace(/<div onMouseEnter=\{sound\.hover\} onClick=\{\(\) => \{ sound\.click\(\); setSelectedGame\('blue_tower'\); \}\}.*?<img src=\{GAME_DETAILS\['blue_tower'\]\.icon\}.*?<\/div>.*?<\/div>/s, blueTowerCard.trim());

const eatClickerCard = `
...
`;
code = code.replace(/<div onMouseEnter=\{sound\.hover\} onClick=\{\(\) => \{ sound\.click\(\); setSelectedGame\('eat_clicker'\); \}\}.*?<img src=\{GAME_DETAILS\['eat_clicker'\]\.icon\}.*?<\/div>.*?<\/div>/s, eatClickerCard.trim());

// 3. Remove GameDetailModal component usage
code = code.replace(/\{selectedGame && GAME_DETAILS\[selectedGame\] && \([\s\S]*?<\/GameDetailModal>\n\s*\)\}/, "");

// 4. Add original inline modals for games
...
if (insertIndex !== -1) {
    code = code.substring(0, insertIndex) + originalModals + code.substring(insertIndex);
}
